...
Equation \ref{eq:duty} is one of the most important equations in boost converter design, as it defines the relationship between the input voltage, output voltage and duty cycle. As the duty cycle increases, the output voltage also increases, which is the main principle behind boost converters. However, this equation is only valid for ideal components since it does not take into account the parasitic elements of the components, such as the resistance of the inductor and the voltage drop across the diode. In a real boost converter, these parasitic elements will cause a voltage drop and therefore reduce the output voltage.
...
\begin{equation}
    V_{Lavg} = \frac{1}{T} (\int_{0}^{DT} V_{in} dt + \int_{DT}^{T} (V_{in} - V_{out}) dt) \Leftrightarrow R_L \cdot I_L = \frac{1}{T} [T \cdot D \cdot  V_{in} + T \cdot (1-D)\cdot(V_{in} - V_{out} -V_d)] \Leftrightarrow
\end{equation}
...
Equation \ref{eq:vout2} shows the 2 mains causes of losses in a boost converter. The main cause is the voltage drop across the diode, which causes a significant reduction in the output voltage (usually 0.3~V to 0.7~V). This will be solved by using a synchronous boost converter (Figure~\ref{fig:sync_boost}), which replaces the diode with a transistor. In synchronous boost converters the voltage drop across the transistor is much smaller due to the low $R_{DS,on}$, usually lower than 0.1~V.

As for the inductor internal resistance, it is not possible to remove it intearly, so it must be taken into account when designing the boost converter.
...
\begin{equation}
    MAX[(1-D)^2\cdot D]= (1-0.3(3))^2 \cdot 0.3(3)= \frac{4}{27}
\end{equation}
\begin{equation}
...
\textcolor{red}{Não sei até que ponto é que faz sentido usar o maximo mas o minimo também não dá nenhum valor de jeito. Provavelmete o que irei usar é o valor em steady state tendo como input 40 celulas. Também vi uma paper que usava o valor minimo de corrente que será atingido durante a fase CV do charging, mas acho que não faz muito sentido.}

This is a reference value, if it were used it would highly stress the components, generating heat, magnetic interference and high current ripples. Therefore, the calculation was repeated using a ripple factor of 0.4. \textcolor{red}{Não tenho a ceteza se este é um bom valor. Um paper online diz para usar um valore entre 0.3 e 0.6, mas não explica muito bem o porquê.} 
...
\subsubsection{Capacitor}
    ...
    Additionally, the output and input needs to filtered to reduce the high frequency noise generated by the switching. This is achieved by adding a small capacitor in parallel with the main capacitors. The values use were 1~$\mu F$ and 0.1~$\mu F$. \textcolor{red}{Supostamente os condesadores ceramicos são melhores para isto devio a um ESR baixo. Terei de pesquisar sobre o assunto.}

\subsubsection{Transistor and gate driver}
    The transistor is the main switching device on a synchronous boost converter. The choice of the transistor will highly influence the behavior and performance of the converter.
