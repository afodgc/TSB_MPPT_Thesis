...
\section{Boost}
    ...
    \subsection{Temperature rise}
        ...
        The results are close to what was expected. In Section~\ref{subsubsec:loss_estimation}, we concluded that the component with the highest losses is the inductor, followed by the capacitors and the transistors. In this section, we observe that the temperatures of these three components follow the same order. This makes sense, since the temperature of these components is related to their losses. However, the estimated losses for the inductors and capacitors are very similar, but the temperature of the inductor is higher. This can be explained by the fact that the inductor has a smaller surface area, and therefore it is more difficult to dissipate heat.
        ...
        \begin{figure}[!ht] 
            ...
            \caption{Temperature evolution during 3 hours. \textcolor{red}{Preciso do DBC para adicionar o Tfets e o Tamb}} 
            \label{fig:3h_temp_test}
        \end{figure} 
        
        From Figure~\ref{fig:3h_temp_test}, it can be concluded that after 40~min at maximum power, all the temperatures stop rising. \textcolor{red}{acabar de concluir isto quando a figura estiver certa}

        With this test it is concluded that there is no need for cooling. At least in the same conditions of the test. If the device is place inside a box/compartment, the temperature must be monitored and if it rises to a concerning level, the user must add force ventilation. 

        \textcolor{red}{talvez repetir o teste com uma caixa de cartão a tampara o MPPT}
