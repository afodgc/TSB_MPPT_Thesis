...
\section{Solar Panels}
    ...
    \acs{pv} panels connected in series operate at higher voltage levels, which reduces conduction losses in interconnecting cables and improves the efficiency of the \acs{dc}-\acs{dc} conversion stage. 
    ...
    The characteristic curves of a solar panel depend on various factors, including the quality of the materials used, the solar cells, and environmental conditions such as temperature, solar irradiance, wind, and shades. It is noted that under partial shading conditions, it is possible to have multiple local maximums, but overall, there is still only one true \ac{mpp}~\cite{Comparison_PV_Array_MPPT_Techniques_2007}.
...
\section{Maximum Power Point Tracker (MPPT)}
    ...
    There are two main types of "trackers", the mechanical trackers and the electrical trackers, Both aim to maximize the energy extracted from a photovoltaic panel, but in different ways. Mechanical trackers are movable structures that follow the sun's movement, maximizing the panel's exposure to sunlight throughout the day. These devices are complex, expensive, and inefficient, which is why most research has focused on electrical “tracking” of the \ac{mpp}.
...
\section{MPPT Algorithms}
    ...
    \subsection{Incremental Conductance (IncCond)}
        ...
        This algorithm is based on the fact that at the \ac{mpp}, the derivative of power with respect to voltage is zero. By knowing the output voltage and current of the solar panel, the algorithm can calculate the conductance (I/V) and the incremental conductance(dI/dV)~\cite{Intelligent_PV_Module_2006}. 
        ...
        Note: Figure~\ref{fig:IncCond_flowchart} was made for a buck converter, so the duty cycle actions are inverted for a boost converter. For example, if the operating point is to the left of the \ac{mpp}, the duty cycle should be decreased to increase the voltage of the solar panel and move towards the \ac{mpp}.
        ...
        \subsection{Look Up Table (LUT)}
            ...
            The main advantage of this method is its speed since it can reach the \ac{mpp} in one clock cycle. This makes it suitable for applications where fast tracking is required, or weak microcontrollers are in use. However, it is not very accurate~\cite{Look-Up-Table_VS_PeO}.
...
\section{Types of DC-DC Converters}
    ...
    \subsection{Comparison between Non-isolated DC-DC converters}
        ...
        For \acs{mppt} controllers, the choice is mainly governed by the possibility of operating at the \ac{mpp} which is connected with the converter type (Step Up, Down or Down/Up). Also, efficiency, cost (related to the number of storage elements), output voltage ripple are relevant in this choice.
