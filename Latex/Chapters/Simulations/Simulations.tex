...
\label{chap:sims}
    ...
    \label{subsubsect:Sim_Boost}
        ...
        Considering a solar panel operating at his \ac{mpp} with 35 cells, five points of CC-CV charging curve were chosen to test the performance of the converter. These five points are represented in Figure~\ref{fig:cc-cv points}.
        ...
        It is important to note that in this CC-CV curve the constant current is given by the solar panel and not by the battery as it is usually expected. This is only truth due to the maximum charge current of the battery being lower than the current that can be provided by the solar panel. So at CC, the converter is working at \ac{mpp}. Also, the timescale will depend on the battery capacity.

        With the chosen five points, some simulations were carried using capacitors and inductors that would fit the requirements calculated earlier. The objective of these simulations is to optimize the performance of the converter, leading to better choices.
        ...
        Both voltage ripples are lower than what was defined as the goal in Table~\ref{tab:converter_constraints}. As for the current ripple it is still reasonable. The losses/efficiency are also higher than what was aimed, reaching efficiency values up to 99.4\%. 
    ...
    \section{Power Loss Estimation}
        ...
        The deadtime losses are calculated using Equation~\ref{eq:pdead}. These losses originate from the conduction of the reverse conduction path during the dead time interval required to prevent shoot through. This reverse condition path is either from the third quadrant conduction from the GaN device or from the parallel schottky diode added later in version 1.1 of the prototype.
        ...
        Last but not least, the rest of the electronics (\acs{mcu}, \acs{dc}-\acs{dc}, sensors, etc.) were considered too. A sum of the maximum current of every component was made. Then the power losses can be estimated based on the power and efficiency of each \acs{dc}-\acs{dc} converter, Equation~\ref{eq:aux_losses}.
        ...
        Figure~\ref{fig:loss_breakdown} presents the resulting power loss distribution. The calculated total power loss is approximately 1.98~W, corresponding to an efficiency of 98.50\%. The efficiency dropped around 1\%, compared to LTspice simulations and it is expected to drop even further in real laboratory results since the \ac{pcb} will generate losses too.

        \begin{figure}[!ht]
            \centering
            \includegraphics[width=0.6\textwidth]{Images/Implementation/fig_loss_breakdown.pdf}
            \caption{Prototype analytical power loss distribution at Case~2.}
            \label{fig:loss_breakdown}
        \end{figure}

        The largest contributors to the total losses are the \acs{dc} losses of the inductor and the ESR losses of the capacitors. This highlights the importance of selecting an inductor with low winding resistance and capacitors with low \ac{esr}, as these passive components represent a significant portion of the converter losses.

        The auxiliary electronics also have a considerable impact on the overall losses, primarily due to the losses associated with the \acs{dc}-\acs{dc} converter.

        The conduction losses of the GaN transistors account for approximately 24.4\% of the total losses, further highlighting the importance of selecting transistors with low on-state resistance.

        Additionally, the interconnections and traces on the \ac{pcb} exhibit parasitic inductance, which contributes to the overall losses of the device, which were not taken into account in this analysis.

        Overall, the analysis indicates that nearly half of the converter losses originate from the passive components, while the semiconductor devices account for approximately one quarter of the total losses.
        Nevertheless, both the simulation results and theoretical analysis indicate that the selected components and design approach result in a highly efficient converter.
