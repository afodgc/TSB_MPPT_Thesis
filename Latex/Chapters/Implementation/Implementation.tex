...
With the current battery (5844.56 Wh) and the maximum power that the solar panels can currently provide (420.5 W), the minimum time to fully charge the battery is 13h and 54min (5844.56 Wh / 420.5 W). This is a large time to charge a battery. There are two solutions to this problem: increase the power provided by the solar panels or decrease the battery capacity. Both solution can be implemented in this vessel. The battery capacity is over dimensioned since it was designed for another vessel and reutilized in this project. As for the solar panels, there is more usable area in the vessel to install more solar panels, which can increase the power provided by the solar panels to approximately 900 W.

\textcolor{red}{Add render with all solar panels installed.}

Let's consider the case where we reduced the battery capacity to 1512 Wh which was the capacity of our solar boat (\acs{SR03}) battery and will be the battery used for laboratory tests. With this change alone the battery is fully charged in 3h and 35min, which is a much more reasonable time. Additionally, if we increase the power provided by the solar panels to 900 W, the battery will be fully charged in 1h and 41min, which is an excellent time to charge a battery.
